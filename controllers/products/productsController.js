...
const getDailyRateController = async (req, res) => {
  ...
  const { notAllowedProducts, notAllowedProductsAll } = await notAllowedProductsObj(
    req.body.bloodType
  );
...
const getAllProductsByQuery = async (req, res, next) => {
  ...
  if (products.length === 0) {
    const newProducts = await Product.find({
      $or: [{ 'title.en': { $regex: titleFromUrl, $options: 'i' } }],
    }).limit(limit);

    if (newProducts.length === 0) return next(createNotFoundError());
