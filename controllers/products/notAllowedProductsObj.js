...
const notAllowedProductsObj = async bloodType => {
  ...
  notAllowedProductsArray.map(({ title }) => arr.push(title.en));
  ...
  if (notAllowedProductsAll[0] === undefined) notAllowedProducts = message;
  else {
    do {
      ...
      if (
        ...
        break;
      else notAllowedProducts.push(notAllowedProductsAll[index]);
    } while (notAllowedProducts.length !== 5);
  }

  if (notAllowedProductsAll.length === 0) notAllowedProductsAll = message;
