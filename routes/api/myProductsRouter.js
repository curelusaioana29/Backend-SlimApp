...
router.use(auth);
router.post('/addProduct', validation(joiAddMyProductSchema), ctrlWrapper(ctrl.addMyProducts));
router.delete(
  '/:productId',
  validation(joiDeleteMyProductSchema),
  ctrlWrapper(ctrl.deleteMyProducts)
);
router.post('/listMyProduct', validation(joiGetMyProductSchema), ctrlWrapper(ctrl.getMyProducts));
